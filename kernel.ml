...
let pa_least_sort the_PTS p e t =
...
let pa_least_prod the_PTS p e t =
...
let pa_le_type_dec the_PTS p e u v =
...
let ptc_inf_ppal_type the_PTS p e t =
...
let ptc_chk_wft the_PTS p e t =
  p.ptc_chk_wft e t __

(** val not_topsort :
    'a1 pTS_sub_spec -> 'a1 pTS_algos -> 'a1 term -> 'a1 sig2 option **)

let not_topsort the_PTS the_algos = function
| Srt s ->
...
| Expected_type (v, tv, t) -> Apply_err (u, tu, v, tv)
| _ -> e
...
(** val infer_sort :
    'a1 pTS_sub_spec -> 'a1 pTS_algos -> 'a1 env -> 'a1 -> 'a1
    infer_ppal_type **)

let infer_sort the_PTS the_algos e s =
  match the_algos.pa_infer_axiom s with
  | Some x -> Inl (Srt x)
  | None -> Inr (Topsort s)
...
let scts_convert_hn the_CTS s e x y =
...
let cumul_infer_axiom the_CTS the_ncts =
  let inf_axiom = the_ncts.ncts_axiom in
  (fun s ->
  match inf_axiom s with
  | Some x -> Some x
  | None -> None)
...
let cumul_infer_rule the_CTS the_scts the_ncts =
  let inf_rule = the_ncts.ncts_rule in (fun x1 x2 -> inf_rule x1 x2)
...
let cts_prim_algos the_CTS the_scts the_ncts =
  { pa_lift = (fun n t -> lift n t); pa_subst = (fun u v -> subst u v);
    pa_infer_axiom = (cumul_infer_axiom the_CTS the_ncts); pa_least_sort =
    (fun x x0 _ -> cumul_least_sort the_CTS the_scts the_ncts x x0);
    pa_infer_rule = (cumul_infer_rule the_CTS the_scts the_ncts);
    pa_least_prod = (fun x x0 _ ->
    cumul_least_prod the_CTS the_scts the_ncts x x0); pa_le_type_dec =
